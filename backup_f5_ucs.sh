...
# Function to ensure the download directory exists and has the correct permissions
ensure_download_dir() {
...
save_running_config() {
    ...
    save_config_response=$(curl -sku "$F5_USER:$F5_PASS" -H "Content-Type: application/json" -X POST "https://$F5_HOST/mgmt/tm/sys/config" -d '{"command":"save"}')
...
backup_f5() {
    ...
    create_backup_response=$(curl -sku "$F5_USER:$F5_PASS" -H "Content-Type: application/json" -X POST "https://$F5_HOST/mgmt/tm/sys/ucs" -d "{\"command\":\"save\",\"name\":\"$BACKUP_NAME\"}")
    ...
    #curl -sku "$F5_USER:$F5_PASS" "https://$F5_HOST/mgmt/shared/file-transfer/ucs-downloads/$BACKUP_NAME.ucs" -o "$DOWNLOAD_DIR/$BACKUP_NAME.ucs"
...
delete_old_backups() {
    ...
    for ucs in $ucs_list; do
        delete_response=$(curl -sku "$F5_USER:$F5_PASS" -H "Content-Type: application/json" -X DELETE "https://$F5_HOST/mgmt/tm/sys/ucs/$ucs")
